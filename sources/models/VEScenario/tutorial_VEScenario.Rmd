...
After creating this database when you run a sample visioneval model, a directory with all the scneario levels and required inputs will be created . see `runtime/VEScenario/scenario_inputs` for an example of the directory structure.
## Running VEScenario
...
VEScenario::NewScenarioSetInterior(ScenarioInputFolder=NULL,ScenarioManagerFile,ModelFolder,ScenarioBaselineInputFolder=NULL)
```                                                                                                     This function requires 4 inputs by user:

** ScenarioInputFolder — a file path identifying the location on the computer hard disk where the inputs for the scenarios will be located.
May be NULL, in which case ScenarioManagerFile is interpreted as the full path to the file, and ** ScenarioManagerFile is computed as basename(normalizePath(ScenarioInputFolder))
** ScenarioManagerFile — the file path of the CSV file described above, which is expected to be in the ScenarioInputFolder. 
...
** ScenarioBaselineInputFolder — the location on the computer of the hard disk where the inputs used for the baseline scenarios are located.
May be NULL, in which case it is constructed as file.path(ModelFolder,”inputs”)
